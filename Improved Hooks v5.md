# Build a Token‑Efficient Hook Automation System for Claude Code (v5)
...
```
 ...
 EVENTS                 │  SessionStart   UserPromptSubmit   PreToolUse   PostToolUse   Stop/SessionEnd    │
                        │  + PreCompact / PostCompact (optional integration)                              │
                        └───────┬───────────────┬────────────────┬─────────────┬──────────────┬───────────┘
                                │ stdin JSON    │                │             │              │
                                ▼               ▼                ▼             ▼              ▼
 ...
 HOOK SCRIPTS           │  context.sh      limit-bridge.sh   guards.sh     format.sh      finish.sh       │
                        ...
                        │   optionally  │                                  (track writes) quarantine)     │
                        │   on SessionEnd                               │                                  │
                        └───────┬───────┼──────────┬──────────────┬─────────────┬──────────────┬──────────┘
                                │       │          │              │             │              │
                                ▼       │          ▼              ▼             ▼              ▼
...
*Notes:*  
- `statusline.sh` writes current usage percentages to `state/<project-key>/limits.json` **to stdout** (per Claude's statusline protocol).  
- `limit-bridge.sh` reads that file to know when to inject tier directives.  
- The `janitor.sh` script may also be triggered via the `Notification` event (with matcher `agent_completed`) for immediate cleanup after session end.
...
| `Stop` / `SessionEnd` | — | `finish.sh` | Repo tidiness: classify manifest entries as scratch, quarantine, report count. | No | 30 |
...
| **Optional** `Notification` | `agent_completed` | `janitor.sh` | Run janitor immediately after session ends (complements the `SessionStart` rate‑limited run). | No | 60 |
...
- Invoked by Claude Code on every refresh (via `statusLine` setting).  
  *Refresh triggers*: After each assistant message, after `/compact` completes, when permission mode changes. Claude applies a built‑in 300 ms debounce; you cannot control this.
...
- Prints its display to **stdout** (per Claude’s statusline protocol). The display should include: current model, branch + dirty count, last report status, and the three percentages with tiered colour coding (🟡 / 🟠 / 🔴).
...
- `janitor.sh` runs on `SessionStart`, rate‑limited to once per `janitor_interval_hours` (global config).  
  **Optionally**, you may also run it on the `Notification` event with `matcher: agent_completed` to trigger cleanup immediately after a session ends (this is not rate‑limited but can be made idempotent).
...
1. **`settings.json`** — Hook configuration with narrow matchers and explicit `timeout` fields.
...
6. **`hooks/README.md`** — Table of hooks, config layering, how to add an overlay, auto‑compact recommendation (`CLAUDE_AUTOCOMPACT_PCT_OVERRIDE=85`), notes on version requirements for `rate_limits`, and explanation of statusline refresh vs. state write throttling.
...
- [ ] Janitor removes aged files, respects size cap, runs ≤ once per `janitor_interval_hours` (plus optional post‑session run), and cleans up session manifests.
