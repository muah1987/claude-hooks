# Build a Token-Efficient Hook Automation System for Claude Code (v15)

> **Canonical location**: `~/.claude/hooks/SPEC.md` (this file, living with the implementation
> it specifies). Copies elsewhere (e.g. `D:\Projects\Claude-Hooks\Improved Hooks v15.md`) are
> **archives** — when the two differ, this file wins. Refresh the archive on every version bump.

## Summary of Changes from v14

All eight items from the v14 spec review ("Improved Hooks v14 spec review.md", 2026-07-17), applied:

| # | Area | v15 change |
|---|------|-----------|
| 1 | Seventh filter verified | `Bash(*/dev/*)` mid-string matching **empirically confirmed** `[captured 2026-07-17: spawn probe — a temporary deny pattern fired on a command containing /dev/ that no other filter matches]`. Rule 8 now also specifies the fallback for deny families with no command name: run the guard **unfiltered** rather than guessing at glob semantics. |
| 2 | §10.E executable | The family-coverage cross-check is an executable test in `test/run.sh` (deliverable 12), not reviewer prose — the v14 draft violated its own rule within minutes of writing it, proof the check must be automated. |
| 3 | Canonical spec location | Declared above; archive copies carry a provenance header. |
| 4 | Live-session criteria tagged | §10 criteria that cannot be fixture-tested are tagged `[live-session]`. |
| 5 | §5.4 split | Validator target types split into a **minimum probe set** (acceptance-tested) and a **full probe set** (roadmap). |
| 6 | Cold-start as budget | §6.2 cold-start figure rephrased as a budget (≤ 2.5× the warm target), not a portable measurement. |
| 7 | Version history citations | §12 cites the prior spec files by name. |
| 8 | Evidence-tag convention | New ground rule 14: every schema/behavior claim carries `[docs YYYY-MM-DD]`, `[captured <fixture/probe>]`, or `[UNVERIFIED]`. In this series, a claim without evidence had a half-life of ~2 revisions (stdout→stderr→stdout across v5→v6→v7; architecture inverted in v10; `if` regressed to regex in v11). |

Provenance: v14's two defects were found by implementing v13; v15's changes come from the v14 spec review conducted against all thirteen prior documents plus the running reference implementation (65-assertion suite, Claude Code 2.1.208).

---

# Full v15 Specification
...
8.  **Safety**: Never auto-approve destructive operations. `PreToolUse` may auto-allow **only** an explicit allowlist of read-only or idempotent commands. Enforcement guards live on `PreToolUse` (the only tool event that can prevent execution). The `if` pre-filter is best-effort and **fails open** (see §1.6), so the guard script itself always re-validates against config; for hard, non-negotiable denies, prefer the native permission system (`permissions.deny` rules) over hooks and note this in the README. **Filter/pattern co-maintenance (v14, hardened in v15)**: every deny-pattern *family* in `config.json → guards.*` must be covered by a corresponding `if` filter on a guard handler in `settings.json` (or by an unfiltered handler on the same matcher). A config pattern whose command family no `if` filter matches will never spawn the guard—the optimization becomes a bypass, and it fails **silent**, not open. Whenever a pattern family is added to config, its `if` entry is added in the same change. For a family with **no command name to filter on** (e.g. output redirection to a device), do not guess at glob semantics: either use a filter whose matching behavior is evidence-tagged (rule 14) — `Bash(*/dev/*)` is `[captured 2026-07-17: spawn probe]` — or run the guard unfiltered on that matcher and accept the spawn cost. This coverage requirement is **enforced by an executable cross-check in `test/run.sh`** (§10.E), not by review.
9.  **Loop Guard**: Any `Stop` hook must check the `stop_hook_active` field in the stdin JSON and `exit 0` when `true`. (Empirically confirmed present on real `Stop` payloads even where the rendered docs truncate; read it defensively with a `// false` fallback regardless.)
...
11. **Zero Hardcoded Values**:
    ...
    - **Portability**: All scripts must run on macOS and Linux (and Windows via WSL/Git Bash) without modification. Use POSIX-compliant constructs or `#!/usr/bin/env bash`. BusyBox userlands lack GNU extensions (`date %N`, `flock -w`, `\s` in ERE)—avoid them.
...
14. **Evidence Tags (new in v15)**: every schema or platform-behavior claim in this document carries one of three tags: `[docs YYYY-MM-DD]` (verified against the official documentation on that date), `[captured <fixture/probe>]` (verified against a captured real payload or a live probe, preferred where docs are truncated), or `[UNVERIFIED]` (confident memory — treat as a to-verify item, never as fact). History justifies the ceremony: across v1–v13 of this spec, three independent correct claims were each "corrected" into wrong ones by revision passes that had no way to distinguish evidence from memory (statusline stdout→stderr→stdout across v5→v6→v7; the limit architecture inverted in v10 after the base file had it right; `if` syntax regressed to regex in v11). A claim without recorded evidence has a demonstrated half-life of about two revisions. When revising this spec, never weaken a tagged claim without new evidence of at least equal strength; when adding a claim you cannot tag, tag it `[UNVERIFIED]` rather than omitting the tag. Schema excerpts in §1.1–§1.8 are collectively `[docs 2026-07-15]` + `[captured statusline-real.json, stop-real probe 2026-07-17]` unless marked otherwise.
...
Event-specific fields are added per event, e.g. `tool_name`, `tool_input`, `tool_use_id` on `PreToolUse`; `tool_response` on `PostToolUse`; `stop_hook_active` and `last_assistant_message` on `Stop`; `source` on `SessionStart`. `prompt_id` and `effort` may be absent on older versions or certain events—read all fields defensively. Real payloads may also carry additional undocumented fields (e.g. `background_tasks`, `session_crons` observed on `Stop`)—ignore unknown fields, never fail on them.

**Hook stdin does NOT contain `context_window` or `rate_limits`.** Those fields are delivered only to the **statusline** command (§1.2). Any design that expects usage metrics inside a hook must read them from the state file the statusline maintains (§6). Do not "correct" this at implementation time based on remembered schemas—verify against the live docs; this exact inversion was the critical defect of spec versions v10–v12.
...
**Critical facts (the exact opposite of what v10–v12 claimed; confirmed by captured payload on 2.1.208):**
...
- The statusline **does** receive `rate_limits.five_hour` / `rate_limits.seven_day` (each with `used_percentage` and `resets_at` in epoch seconds) on subscription plans and recent versions. The field can be **absent**: on API-key billing, on some versions (there has been at least one regression that dropped it), and in the first renders after `/clear`. Handle absence gracefully.
...
Therefore: use `if` purely as a **spawn-count optimization**, keep the filter set in lock-step with the config deny families (rule 8), the guard script must always re-validate the full command against the configured patterns, and hard denies belong in the native permission system.
...
- `timestamp`: ISO-8601 time of the write; consumers may equivalently use the file's mtime to detect staleness.
...
- **Guard patterns** (`guards.bash_deny` and related keys): dangerous Bash patterns (recursive delete outside tmp, force-push to the detected default branch, `chmod 777`, writes to `/dev/`...). These live **here**, not in `settings.json`. Each pattern family must have a matching `if` filter in the settings wiring (rule 8).
...
The user-level `~/.claude/settings.json` follows this exact structure. **Note the seven `if`-filtered Bash guard entries — one per deny-pattern family in the shipped `config.json` (rule 8):**
    ...
    "PreToolUse": [
      {
        "matcher": "Bash",
        "hooks": [
          { "type": "command", "command": "~/.claude/hooks/guards.sh", "timeout": 5, "if": "Bash(rm *)" },
          { "type": "command", "command": "~/.claude/hooks/guards.sh", "timeout": 5, "if": "Bash(git push *)" },
          { "type": "command", "command": "~/.claude/hooks/guards.sh", "timeout": 5, "if": "Bash(sudo *)" },
          { "type": "command", "command": "~/.claude/hooks/guards.sh", "timeout": 5, "if": "Bash(dd *)" },
          { "type": "command", "command": "~/.claude/hooks/guards.sh", "timeout": 5, "if": "Bash(mkfs*)" },
          { "type": "command", "command": "~/.claude/hooks/guards.sh", "timeout": 5, "if": "Bash(chmod *)" },
          { "type": "command", "command": "~/.claude/hooks/guards.sh", "timeout": 5, "if": "Bash(*/dev/*)" }
        ]
      },
      {
...
- `if` uses **permission-rule syntax** (§1.6). The six `Bash(...)` filters above are spawn optimizations that must jointly cover every `guards.*` deny family in config (rule 8); `guards.sh` still re-checks every command against `config.json → guards.*`. The `Read|Edit|Write` protected-path guard runs unconditionally (no `if`).
...
| `PreToolUse` | `Bash` | `guards.sh` | Block dangerous patterns (recursive deletes, force-push to detected default branch, device writes, mkfs, chmod-777-on-root, sudo-recursive-delete). Patterns from config. | Yes (`exit 2`) | 5 | Seven `if` filters — `Bash(rm *)` / `Bash(git push *)` / `Bash(sudo *)` / `Bash(dd *)` / `Bash(mkfs*)` / `Bash(chmod *)` / `Bash(*/dev/*)` (redirect-to-device family, which has no command name to filter on) — jointly covering every config deny family (rule 8); script re-checks |
...
Split (new in v15) into what §10 actually tests versus what a mature installation grows into. The **minimum probe set is the requirement**; the full set is roadmap, and a validator that implements only the minimum set reports honestly at that level (e.g. the web validator's curl fallback says so in its summary) rather than pretending to the full set.

**Minimum probe set (acceptance-tested, must ship):**
- **VPS/Server**: SSH connectivity (BatchMode, key auth only), optional service-active check, optional port-listening check, disk headroom.
- **Email**: SMTP banner reachability; MX lookup for the domain; probe send **only** to an explicitly configured sandbox recipient.
- **Web**: page reachability + HTTP status; with Playwright available, page load + no console errors + optional selector present.
- **API**: endpoint status code, optional JSON-body key check.
- **Database**: read-only connectivity (`SELECT 1`), optional migration-version read, optional row-count sanity.

**Full probe set (roadmap, not acceptance-gated):**
- **VPS/Server**: artifact version/checksum comparison, memory pressure, recent service logs scanned for errors.
- **Email**: delivery verification to the sandbox inbox, SPF/DKIM/DMARC alignment, unintended-recipient audit.
- **Web**: network-request assertions, critical user flows, screenshots saved to `logs/<key>/screenshots/`.
- **API**: schema validation against the project's OpenAPI document, auth-behaviour probes.
- **Database**: migration-drift detection against the repo's migration directory.
...
- **Performance (revised in v14, rephrased as a budget in v15)**: target **< 100 ms warm** — i.e., when the mandatory ≥5-second cache for git status and hook-log counts is hot. The **cold-start budget is 2.5× the warm target** (≤ 250 ms at the current 100 ms), acknowledging that on shell-based implementations fork/exec of the jq/git toolchain dominates and cannot be optimized away (one Alpine/BusyBox data point: ~195 ms — a measurement, not a portable guarantee). An implementation that exceeds the cold budget on its platform should move to a compiled statusline — not tighter shell golf.
- Missing/stale state → placeholders, never errors, never a blank line (a blank/erroring statusline disappears from the UI).
...
- Runs on **`UserPromptSubmit`** (wired in §3.3). Reads `limits.json`; if its `timestamp`/mtime is older than `limits_staleness_minutes` (default 10), treats values as unknown (`n/a` behaviour, no directives).
...
1. **`settings.json`** — Hook configuration exactly as §3.3: narrow matchers, explicit `timeout` on every entry, permission-rule `if` filters on the Bash guards **covering every config deny family**, `UserPromptSubmit` → `limit-bridge.sh`, `statusLine` entry.
...
4. **`hooks/config.json`** — Global defaults with a `"version"` field and all keys documented, including `guards.*`, `env_allowlist`, `test_command`, `test_timeout`, `validator_async_threshold_seconds`, `validation.file_map`, and `limits_staleness_minutes`.
...
6. **`hooks/README.md`** — Table of hooks; config layering; how to add an overlay; the dynamic markdown validation logic and its single block/skip rule; statusline dual role (collector + display, stdout output); `if` permission-rule syntax with the fails-open caveat, the filter/pattern co-maintenance rule, and the permission-system recommendation for hard denies; `args` exec form; `async`/`asyncRewake` semantics (settings-level fields); the "JSON only on exit 0" rule; atomic write guidance; `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE=85` recommendation; note that `PermissionRequest` does not fire in headless mode.
7. **`hooks/fixtures/`** — Sample stdin JSON per hook (including malformed JSON, a statusline payload **without** `rate_limits`, and a Stop payload with `stop_hook_active: true`). Prefer **captured real payloads** over synthetic ones wherever obtainable.
...
9. **`hooks/statusline.sh`** — Dual-role statusline (renders to stdout; writes `limits.json` atomically/throttled; reads `last_status.json`; derives git info; <100 ms warm per §6.2; never blank).
...
- [ ] `[live-session]` `claude --debug` (or namespaced log/state evidence from a fresh headless session) shows each hook firing on its intended event and no others; no hook fires on events it isn't registered for.
...
- [ ] **(new in v14)** Bash guard blocks at least one dangerous command from a *non-rm, non-push* deny family (e.g. `dd` writing to a device, or `mkfs`), proving the `if` filter set covers every config deny family — a family whose filter is missing would never spawn the guard.
...
- [ ] Statusline renders from state (per §6.2 targets), writes `limits.json` atomically and no more than once per 5 s, outputs to **stdout**, and never renders blank on missing state.
...
- [ ] Optional `PermissionRequest` helper auto-approves allowlisted commands and stays silent otherwise; documented as inactive in headless mode (`[UNVERIFIED]` — the docs are silent on this; treat the caveat as defensive documentation, and verify interactively if the helper becomes load-bearing). Optional `PermissionDenied` handler emits `hookSpecificOutput.retry: true` and degrades silently on versions without the event.
...
- [ ] `[live-session]` Compaction completes successfully at the 85 % context threshold (headroom proven; requires a genuinely long session — not fixture-testable).
- [ ] Statusline: **< 100 ms warm** (hot cache); **≤ 250 ms cold start** on BusyBox/musl systems; `limits.json` writes throttled to ≤ once per 5 s; atomic writes cause no corruption under rapid refresh.
...
- [ ] **(v14, made executable in v15)** Every pattern family in `config.json → guards.*` has a covering `if` filter (or unfiltered handler) in `settings.json` — verified by an **executable cross-check in `test/run.sh`** that derives the command families from the config patterns mechanically and fails the suite on any uncovered family. Manual review does not satisfy this criterion (the v14 draft violated the rule on first contact; only automation holds).
...
Prior versions are archived in `D:\Projects\Claude-Hooks\` and cited by filename below (item 7 of the v15 changes). The July 14 review of base/v10–v12 is `Improved Hooks spec review.md`; the v14 review is `Improved Hooks v14 spec review.md`.

| Version | File | Changes |
|---------|------|---------|
| **base** | `Improved Hooks .md` | Compact prose spec. Had the statusline-bridge limit architecture **right**. |
| **v2–v5** | `Improved Hooks v2 .md` … `v5.md` | Formalization: executive summary, report contract, merge rules, project keys, `file_map`, POSIX portability, malformed-overlay fallback. |
| **v6** | `Improved Hooks v6.md` | Added StopFailure/PostToolUseFailure/Pre+PostCompact; **regressed statusline output to stderr**. |
| **v7** | `Improved Hooks v7.md` | Reverted statusline to stdout; full event catalog; settings schema detail. |
| **v9** | `Improved Hooks v9 .md` | Added dynamic LLM-maintained context files (contains `[cite]` artifacts — likely regenerated from a summary rather than revised from the prior text). |
| **v10** | `Improved Hooks v10.md` | Formal spec structure — and **inverted the limit architecture** the base file had right. |
| **v11** | `Improved Hooks V11.md` | Re-introduced `validation.file_map`; added `last_status.json`; dynamic merge rules; extraction improvements; **regressed `if` from permission-rule syntax to "regex"**; added git info; documented (unsafe) secrets loading; `.claude/` fallback; `asyncRewake`. |
| **v12** | `Improved Hooks V12.md` | Manifest `Write\|Edit`; dynamic merge restricted to `validation` subtree; async validators; test config; `awk` extraction; `limits.json` defined; atomic writes; carried forward the inverted limit architecture, the regex `if`, the PreToolUse `format.sh`, and the unwired bridge. |
| **v13** | `Improved Hooks v13.md` | **Correctness pass against official docs** (executing the July 14 review's fix list): limit architecture restored to statusline-bridge (statusline = collector + display; `limit-collector.sh` deleted); `limit-bridge.sh` wired on `UserPromptSubmit`; hook-input schema corrected (no `context_window`/`rate_limits` in hook stdin) and fabricated version claims removed; `if` restored to permission-rule syntax with fails-open caveat and patterns moved to config; `format.sh` moved to `PostToolUse`; Stop/PostToolUse blocking semantics corrected + "JSON only on exit 0" rule; `PermissionDenied` retry shape fixed; `limits.json` stores `used_percentage`; single block/skip rule for malformed dynamic context keyed on declared validation; canonical manifest path; detached async validators with documented weakened guarantee; safe `.env` parser replacing `source`; FileChanged note removed; naming, path, and table corrections. |
| **v14** | `Improved Hooks v14.md` | **Post-implementation fixes (both defects found by building and verifying v13 on Claude Code 2.1.208)**: (1) guard `if`-filter coverage — §3.3/§4.2 now wire seven filters mirroring every config deny family, with a co-maintenance requirement added to rule 8, §1.6, §3.1 and a new acceptance criterion in §10.A/§10.E; (2) statusline performance target split into <100 ms warm / ≤250 ms cold (BusyBox/musl reality), §6.2 + §10.B. Also: `stop_hook_active` and `last_assistant_message` empirically confirmed on real Stop payloads (rule 9 note); §1.1 notes tolerance of undocumented extra fields; deliverable 7 prefers captured payloads over synthetic fixtures. |
| **v15** | `Improved Hooks v15.md` (archive of `~/.claude/hooks/SPEC.md`, canonical) | **All eight items from the v14 spec review**: `Bash(*/dev/*)` matching empirically verified (spawn probe) + unfiltered-guard fallback rule for command-nameless deny families; §10.E coverage check made an executable test; canonical spec location declared with provenance headers; `[live-session]` tags on non-fixture-testable criteria; §5.4 split into minimum/full probe sets; cold-start rephrased as a 2.5× budget; version history cites archive files by name; **rule 14 evidence-tag convention** (`[docs]`/`[captured]`/`[UNVERIFIED]`) adopted as a ground rule. |

---

**This v15 specification applies the complete fix list from the v14 spec review and carries forward all v13/v14 corrections. Its distinguishing property is that it is self-verifying where its predecessors were self-confident: schema claims carry evidence tags (rule 14), wiring-coverage rules are enforced by an executable test rather than review, and the criteria that genuinely require a live session say so. Re-verify tagged claims against the live docs and fresh captured payloads whenever the platform version changes.**
