# Build a Token-Efficient Hook Automation System for Claude Code (v14)

## Summary of Changes from v13

| Area | v13 (defective) | v14 (fixed) |
|------|-----|-----|
| **Guard spawn-filter coverage** | §3.3 wired the Bash guard behind only two `if` filters (`Bash(rm *)`, `Bash(git push *)`) while the config's own `guards.bash_deny` covered `sudo`/`dd`/`mkfs`/`chmod` patterns — those families could **never spawn the guard**. The optimization silently became a bypass. | Seven `if`-filtered handlers mirroring every deny-pattern family in config; new maintenance rule (rule 8): config deny families and `if` filters are maintained **together** — adding a family without its filter (or an unfiltered handler) is a wiring bug. Acceptance test added for a non-rm dangerous command. |
| **Statusline performance target** | Flat "< 100 ms" — unrealistic for a cold start on BusyBox/musl systems where process spawns dominate (~195 ms measured on Alpine). | Split target: **< 100 ms warm** (cached git/hook-count path, mandatory ≥5 s cache), **≤ 250 ms cold start tolerated** on BusyBox/musl. Acceptance criterion updated to match. |

Provenance: both defects were found while implementing and verifying v13 (2026-07-17). All other v13 content is carried forward unchanged; the v13 schema claims were re-verified against the live docs and against captured payloads on Claude Code 2.1.208 (including empirical confirmation of `stop_hook_active` and `last_assistant_message` on real `Stop` stdin — the docs page truncates before that section).

---

# Full v14 Specification
...
8.  **Safety**: Never auto-approve destructive operations. `PreToolUse` may auto-allow **only** an explicit allowlist of read-only or idempotent commands. Enforcement guards live on `PreToolUse` (the only tool event that can prevent execution). The `if` pre-filter is best-effort and **fails open** (see §1.6), so the guard script itself always re-validates against config; for hard, non-negotiable denies, prefer the native permission system (`permissions.deny` rules) over hooks and note this in the README. **Filter/pattern co-maintenance (new in v14)**: every deny-pattern *family* in `config.json → guards.*` must be covered by a corresponding `if` filter on a guard handler in `settings.json` (or by an unfiltered handler on the same matcher). A config pattern whose command family no `if` filter matches will never spawn the guard—the optimization becomes a bypass. Whenever a pattern family is added to config, its `if` entry is added in the same change; if that discipline cannot be kept, drop the filters and run the guard unfiltered.
...
- **Performance (revised in v14)**: target **< 100 ms warm** — i.e., when the mandatory ≥5-second cache for git status and hook-log counts is hot. A **cold start of up to ~250 ms is tolerated** on BusyBox/musl systems, where fork/exec of the jq/git toolchain dominates and cannot be optimized away in shell (measured ~195 ms on Alpine). If cold-start latency matters more than portability, a compiled statusline is the escape hatch — not tighter shell golf.
...
- [ ] **(new in v14)** Every pattern family in `config.json → guards.*` has a covering `if` filter (or unfiltered handler) in `settings.json` — verified by cross-referencing the two files.
