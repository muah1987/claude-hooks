# Build a Token‑Efficient Hook Automation System for Claude Code (v7)

## Summary of Changes from v6

Based on official Claude Code hooks documentation review, the following corrections and enhancements have been made:

| Area | v6 | v7 |
|------|----|----|
| **Statusline output** | Printed to **stderr** | Printed to **stdout** (correct protocol) |
| **Missing events** | Lacked several official events | Added `Setup`, `UserPromptExpansion`, `PostToolBatch`, `MessageDisplay`, `SubagentStart/Stop`, `TaskCreated/Completed`, `TeammateIdle`, `InstructionsLoaded`, `ConfigChange`, `CwdChanged`, `FileChanged`, `WorktreeCreate/Remove`, `Elicitation/ElicitationResult` |
| **Event categorization** | Not specified | Added official frequency categories (per session/per turn/per tool call) |
| **Config structure** | Basic | Added exact `settings.json` schema, regex matcher support, timeout/async placement |
| **`$CLAUDE_PROJECT_DIR` usage** | Mentioned | Added official usage examples |
| **MCP tool hooks** | Not mentioned | Added out-of-scope note with reasoning |
| **Plugin hook merging** | Brief mention | Added auto‑merge behavior and `${CLAUDE_PLUGIN_ROOT}` reference |
| **Hook type comparison** | Not documented | Added table comparing Command vs Prompt vs HTTP |
| **Enterprise settings** | Not mentioned | Added `allowManagedHooksOnly` note |
...
12. **Layered Configuration**:
    ...
    - **Configuration Levels** (official hierarchy):
        1. `~/.claude/settings.json` — User‑level (this system)
        2. `.claude/settings.json` — Project‑level (repository‑specific)
        3. `.claude/settings.local.json` — Local project‑level (not committed, overrides project)
        - **Enterprise**: Administrators may set `allowManagedHooksOnly: true` to block user/project/plugin hooks. This system respects that setting without requiring special handling.
...
**Official usage pattern for project‑level scripts**:
```json
{ "type": "command", "command": "\"$CLAUDE_PROJECT_DIR\"/.claude/hooks/script.sh" }
```
...
```
 ...
 EVENTS                 │  SessionStart · Setup · UserPromptSubmit · PreToolUse · PostToolUse · Stop       │
                        ...
                        │  + MessageDisplay · SubagentStart/Stop · TaskCreated/Completed                  │
                        │  + TeammateIdle · InstructionsLoaded · ConfigChange · CwdChanged                │
                        │  + FileChanged · WorktreeCreate/Remove · Elicitation/ElicitationResult          │
                        │  + Notification (limited matchers) · SessionEnd                                 │
                        └───────┬───────────────┬────────────────┬─────────────┬──────────────┬───────────┘
                                │ stdin JSON    │                │             │              │
                                ▼               ▼                ▼             ▼              ▼
...
*Notes:*
- `statusline.sh` writes current usage percentages to `state/<project-key>/limits.json` and **prints its display to stdout** (per official protocol).
...
### 2.1 Event Frequency Categories (Official)
...
| **Assistant display** | `MessageDisplay` |
| **Subagent** | `SubagentStart`, `SubagentStop` |
| **Task** | `TaskCreated`, `TaskCompleted` |
| **Agent Team** | `TeammateIdle` |
| **Configuration** | `ConfigChange`, `CwdChanged`, `FileChanged` |
| **Worktree** | `WorktreeCreate`, `WorktreeRemove` |
| **MCP/Elicitation** | `Elicitation`, `ElicitationResult` |
| **Limit monitoring** | `PreCompact`, `PostCompact` |
...
```json
{
  "hooks": {
    ...
    "Stop": [
      {
        "hooks": [
          { "type": "command", "command": "~/.claude/hooks/finish.sh", "timeout": 120 }
        ]
      }
    ],
    "SessionStart": [
    ...
    "SessionEnd": [
      {
        "hooks": [
          { "type": "command", "command": "~/.claude/hooks/finish.sh", "timeout": 30 }
        ]
      }
    ]
  },
...
| `StopFailure` | — | `error-recovery.sh` | Log API error details; no blocking (output ignored). | No | 5 | **New in v7** |
| `PostToolUseFailure` | `Write\|Edit` | `error-recovery.sh` | Log write failure; just log. | No | 5 | **New in v7** |
| `PostToolBatch` | — | `error-recovery.sh` | Log batch completion; optional summary. | No | 5 | **New in v7** |
| `PreCompact` | — | `compact-monitor.sh` | Update state with pre‑compact percentages; inject preservation directive if needed (optional). | No | 5 | |
...
| **Optional** `PermissionRequest` | `Bash` | `permission-helper.sh` | Auto‑allow read‑only/idempotent commands from allowlist; else deny or pass through. | Yes (JSON `permissionDecision`) | 5 | |
| **Optional** `PermissionDenied` | `Bash\|Write\|Edit` | `retry-handler.sh` | If a command was denied, optionally retry with modified arguments or prompt user. | No (output ignored) | 5 | |
...
**Events excluded from this system** (with justification):
...
- `MessageDisplay` — Fires too frequently (per message); would create excessive overhead.
...
- `ConfigChange`/`CwdChanged`/`FileChanged` — Configuration changes are out of scope; handling would add complexity.
...
- `Notification` — Limited matchers; `SessionEnd` covers cleanup needs.
- `Subagent` events — Subagents are out of scope for v1.
...
- Prints its display to **stdout** (per official Claude Code statusline protocol). The display should include: current model, branch + dirty count, last report status, and the three percentages with tiered colour coding (🟡 / 🟠 / 🔴).
...
6. **`hooks/README.md`** — Table of hooks, config layering, how to add an overlay, auto‑compact recommendation (`CLAUDE_AUTOCOMPACT_PCT_OVERRIDE=85`), notes on version requirements for `rate_limits`, explanation of statusline refresh vs. state write throttling, correct output channel (**stdout**), event frequency categories, and official `settings.json` structure reference.
...
- **HTTP, Prompt, and Agent hook types**: This system uses `command` hooks exclusively. Prompt hooks incur LLM cost; HTTP hooks add network dependencies; Agent hooks are for advanced workflows. Command hooks are the most predictable, deterministic, and token‑efficient option for this automation layer.
...
| **v6** | Fixed statusline output to **stderr** (later found incorrect), added `StopFailure`, `PostToolUseFailure`, `PreCompact`, `PostCompact`, moved repo tidiness to `SessionEnd`, added optional `PermissionRequest`/`PermissionDenied`, removed unconfirmed `agent_completed` matcher, documented refresh triggers, added timeout syntax, noted plugin coexistence |
| **v7** | **Fixed statusline output to stdout** (correct protocol), added missing events (`Setup`, `UserPromptExpansion`, `PostToolBatch`, `MessageDisplay`, `SubagentStart/Stop`, `TaskCreated/Completed`, `TeammateIdle`, `InstructionsLoaded`, `ConfigChange`, `CwdChanged`, `FileChanged`, `WorktreeCreate/Remove`, `Elicitation/ElicitationResult`), added event frequency categories, added exact `settings.json` structure with regex matcher and async/placement, added `$CLAUDE_PROJECT_DIR` usage examples, added MCP out‑of‑scope note, added plugin hook merging details, added hook type comparison table, added enterprise `allowManagedHooksOnly` note, added justification for excluded events |

---

**This v7 specification is now aligned with official Claude Code hooks documentation and ready for implementation.**