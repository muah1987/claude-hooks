# Build a Token‑Efficient Hook Automation System for Claude Code (v12)

## Summary of Changes from v11

| Area | v11 | v12 |
|------|-----|-----|
| **Manifest tracking** | Only `Write` (misses edits) | Changed matcher to `Write\|Edit`; track both creations and modifications; only new files are quarantine candidates |
| **Dynamic context merge scope** | Full deep‑merge (security risk) | Restricted to the `"validation"` subtree only; dynamic JSON overrides only that branch |
| **Slow validators** | Always synchronous (blocking) | Re‑introduced async mode: validators with expected runtime > `validator_async_threshold_seconds` (default 15) run in background; results injected on next prompt |
| **Test suite execution** | Undefined | Added config keys `test_command` (discovery heuristic) and `test_timeout`; Stop hook runs tests if command found |
| **JSON extraction from markdown** | Used `perl` (not portable) | Use portable `awk` state machine (POSIX) to extract block between `<validator-targets>` tags |
| **`last_status.json` updates** | Vague | Clarified: only hooks that emit a report summary must update it; listed explicitly |
| **`limits.json` format** | Not defined | Defined exact JSON structure with timestamp |
| **Atomic state file writes** | Not mentioned | Added requirement to write to `.tmp` then rename for atomicity |
| **`settings.json` example** | Manifest matcher `Write` | Corrected to `Write\|Edit` |
| **`if` condition** | Vague | Clarified case‑sensitive regex matching against full tool call string |
| **Default branch detection** | `git remote show origin` (slow) | Recommend `git symbolic-ref refs/remotes/origin/HEAD` with fallback |
| **Test harness** | BATS optional | Added simple runner example with `--test` and JSON validation |
...
11. **Zero Hardcoded Values**:
    ...
    - **Project specifics**: Discover at runtime (e.g., from `package.json`, `Makefile`, `pyproject.toml`). Formatters/linters are detected per project; if none exist, the hook no‑ops silently. Default branch is read from `git symbolic-ref refs/remotes/origin/HEAD` (fallback to `git remote show origin`)—never assumed to be `main`.
...
The `if` field in a hook entry specifies a **regular expression** that is matched against the tool call string (the full command or path). The match is **case‑sensitive** and matches against the entire string unless anchored. For example, `"if": "rm -rf"` will run the hook when the Bash command contains `rm -rf` (substring). To match exactly, use `"if": "^rm -rf .*$"`. Consult the official hooks documentation for the exact matching semantics.
...
The `limit-collector.sh` hook writes to `state/<project-key>/limits.json` with the following structure:
...
- `context`: `context_window.used_percentage` (integer)
- `five_hour`: `rate_limits.five_hour.used` (or `null`/`"n/a"` if unavailable)
- `seven_day`: `rate_limits.seven_day.used` (or `null`/`"n/a"` if unavailable)
...
```
 ...
 HOOK SCRIPTS           │  context.sh      limit-bridge.sh   guards.sh     format.sh      finish.sh       │
                        ...
                        │                 │  (writes limits,               & edits)       hook_event_name)│
                        │                 │   throttled)                                  (async support) │
                        └───────┬───────┼──────────┬──────────────┬─────────────┬──────────────┬──────────┘
                                │       │          │              │             │              │
                                ▼       │          ▼              ▼             ▼              ▼
 ...
 (Stop hook, using      parameters extracted from `<validator-targets>` JSON in validator_context.md,
  filtered by validation.file_map and changed files, with async support for slow validators)
                                │
                                ▼
...
- `limit-collector.sh` (on `PostToolUse`) **writes** `limits.json` from stdin data, with a write throttle (≤ once per 5 seconds) and atomic writes.
...
- Slow validators (estimated runtime > `validator_async_threshold_seconds`, default 15) run asynchronously; their results are written to state and injected on the next UserPromptSubmit.
...
**Extraction Logic**: Use a POSIX‑compliant `awk` state machine to extract content between the opening and closing tags (avoid `perl` for portability). The extracted text must be validated with `jq`. If extraction fails or `jq` rejects it, the hook blocks the stop and instructs Claude to fix the syntax.
...
- `if` condition: case‑sensitive regex matched against the tool call string; only runs if matched
...
| `PostToolUse` | `Write\|Edit` | `limit-collector.sh` | Extract `context_window` and `rate_limits` from stdin, write to `limits.json` (atomic, throttled ≤ once/5s). | No | 5 | |
...
| `Stop` | — | `finish.sh` | Run tests + dynamic validators (filtered by `validation.file_map` and changed files). Block if failures. Slow validators run async. | Yes (`decision: "block"`) | 120 | Branches on `hook_event_name` |
...
1. **Extract**: Use a POSIX `awk` state machine to extract the JSON block between `<validator-targets>` and `</validator-targets>`. If extraction fails, skip remote validation (log warning).
...
6. **Execute validators**:
   - For each target, estimate runtime. If > `validator_async_threshold_seconds`, launch in background (with `asyncRewake` if desired) and store a pending state. Otherwise, run synchronously in parallel with timeouts.
   - Wait for synchronous validators. If any fail, block stop.
   - For async validators, write results to `state/<project-key>/pending_validator_results.json`. On the next UserPromptSubmit, the `limit-bridge.sh` (or a dedicated hook) will inject the results as `additionalContext`.
7. **Block** if any synchronous validator fails: return JSON `decision: "block"` with a summary of failures.
...
- Writes to `state/<project-key>/limits.json` with a **write throttle**: do not write more than once every 5 seconds (use a timestamp file to track last write). Writes must be atomic.
- If `rate_limits` is unavailable (older version), write `null` for those fields.
...
4. **`hooks/config.json`** — Global defaults with a `"version"` field and all keys documented, including `test_command`, `test_timeout`, `validator_async_threshold_seconds`, and `validation.file_map`.
...
6. **`hooks/README.md`** — Table of hooks, config layering, how to add an overlay, explanation of the dynamic markdown validation logic, correct output channel (**stdout**), statusline architecture, `if`/`args`/`shell`/`asyncRewake` usage, version requirements, and atomic write guidance.
...
10. **`hooks/limit-collector.sh`** — Extracts `context_window` and `rate_limits` from stdin, writes to `limits.json` with atomic writes and throttle.
...
- [ ] `limit-collector.sh` correctly writes `limits.json` from stdin data, throttled to ≤ once/5s, atomic; statusline reads it correctly.
...
- [ ] Slow validators (> threshold) run asynchronously; results are injected on next prompt.
...
- [ ] Statusline < 100 ms; state file writes (limit‑collector) throttled to ≤ once per 5 s; atomic writes do not cause corruption.
...
| **v12** | **Critical fixes**: Manifest tracking changed to `Write\|Edit`; dynamic merge restricted to `validation` subtree; re‑introduced async validator support; added test suite configuration; replaced `perl` with `awk`; clarified report‑producing hooks; defined `limits.json` format; added atomic write requirement; updated `settings.json` example; clarified `if` regex; recommended faster default branch detection. |

---

**This v12 specification is fully aligned with official Claude Code hooks documentation, addresses all identified issues, and is ready for implementation.**